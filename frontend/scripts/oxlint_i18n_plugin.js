...
// path.relative uses the platform separator, so the very same mismatch reads
// "locales/de.json" on Linux and "locales\de.json" on Windows. These strings
// are compared verbatim by the plugin tests, so keep them platform-independent.
const relativePosix = (from, to) => path.relative(from, to).split(path.sep).join("/");
    ...
    for (let i = 0; i < locales.length - 1; i++) {
        for (let j = i + 1; j < locales.length; j++) {
            ...
            if (onlyLeft.length > 0 || onlyRight.length > 0) {
                messages.push(
                    `${relativePosix(root, left.filePath)} and ${relativePosix(root, right.filePath)} ` +
            ...
            messages.push(
                `${relativePosix(root, left.filePath)} and ${relativePosix(root, right.filePath)} key order is not aligned: ` +
