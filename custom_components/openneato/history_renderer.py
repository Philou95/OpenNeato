...
def _session_bounds(bounds: dict[str, float], image_size: int):
    ...
    # Hold the viewport open to a sane minimum. A recording session spans only
    # a few centimetres in its first seconds, and fitting that to the canvas
    # asks for hundreds of pixels per metre -- enough to blow a floorplan past
    # the resize ceiling, so the plan silently vanished from the opening
    # frames of every clean and the log filled with "resize too large".
    if world_w < MIN_SESSION_SPAN_M:
        grow = (MIN_SESSION_SPAN_M - world_w) / 2
        min_x, max_x = min_x - grow, max_x + grow
        world_w = MIN_SESSION_SPAN_M
    if world_h < MIN_SESSION_SPAN_M:
        grow = (MIN_SESSION_SPAN_M - world_h) / 2
        min_y, max_y = min_y - grow, max_y + grow
        world_h = MIN_SESSION_SPAN_M
...
def _render_frame(
    ...
    # RGB canvas, not RGBA: PIL only alpha-blends a draw call when the target
    # image is RGB *and* the ImageDraw context was opened in "RGBA" mode. On an
    # RGBA target a fill REPLACES the pixel, alpha included, so the translucent
    # coverage/path colors would punch holes through whatever sits underneath.
    img = Image.new("RGB", (image_size, image_size), HISTORY_BG_COLOR)

    # "RGBA" mode on an RGB image turns on real alpha compositing for every
    # subsequent draw call, so the translucent overlays blend rather than erase.
    ...
    # Already RGB: every overlay was alpha-composited as it was drawn, so
    # there is no separate flattening step to do here.
    return img
